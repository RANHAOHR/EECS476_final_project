...
void DesStatePublisher::initializeServices() {
    ...
    empty_queue_ = nh_.advertiseService("empty_queue_service",
            &DesStatePublisher::emptyQueueCB, this);
}
...
bool DesStatePublisher::emptyQueueCB(mobot_pub_des_state::emptyQueueRequest& request, mobot_pub_des_state::emptyQueueResponse& response) {
    response.empty = path_queue_.empty();
    return true;
}
