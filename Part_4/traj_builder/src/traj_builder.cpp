...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
    ...
    double stop_time =  fabs(speed_x/accel_max_);

    double stop_dist = speed_x*stop_time - 0.5*accel_max_*stop_time*stop_time;
    double dx = cos(psi_des)*stop_dist*sgn(speed_x);
    double dy = sin(psi_des)*stop_dist*sgn(speed_x);

    geometry_msgs::PoseStamped end_pose;
    end_pose.pose.position.x = x_des + dx;
    end_pose.pose.position.y = y_des + dy;

    double speed = fabs(speed_x);
    double spin = spin_z;
    ...
    while(speed > 0){
        ...
        if (speed_x > 0) {
            ...
            des_state.twist.twist.linear.x = speed * -1;
            x_des -= speed * dt_ * cos(psi_des); //Euler one-step integration
            y_des -= speed * dt_ * sin(psi_des); //Euler one-step integration
        }
        des_state.pose.pose.position.x = x_des;
...
void TrajBuilder::build_point_and_go_traj(geometry_msgs::PoseStamped start_pose,
    ...
    ROS_INFO("current heading = %f", current_psi);
    double diff = fabs(fabs(min_dang(current_psi + M_PI)) - fabs(des_psi));
    bool reverse = (diff < 0.14) ? true : false; //pi is 3.14, so if you're within 0.14 or 10°, just back up
    ...
    if (reverse) {
        des_psi = atan2(dy * -1, dx * -1); //if you're backing up, reverse heading by 180°
    }
    ROS_INFO("desired heading to subgoal = %f", des_psi);
    if ((dx == dy) && (dx == 0)) {
        bridge_pose.pose.orientation = end_pose.pose.orientation;
        ...
        if (reverse) {
            for (int i = 0; i < vec_of_states.size(); i++) {
                vec_of_states[i].twist.twist.linear.x *= -1; //if you're backing up, make velocities negative
                vec_of_states[i].pose.pose.orientation = bridge_pose.pose.orientation; //correct orientation
            }
        }    
    }
}
