...
void SteeringController::initializeSubscribers() {
    ...
    backup_subscriber_ = nh_.subscribe("/backup", 1, &SteeringController::toggleBackupCB, this);
}
...
void SteeringController::toggleBackupCB(const nav_msgs::Odometry& odom_rcvd) {  
    ROS_INFO("received dummy state!");
    backUpCalled = true;
    if (shouldBackUp) {
        shouldBackUp = false;
    }
...
int main(int argc, char** argv) 
{
    ...
    while (ros::ok()) {
        ...
        if (!steeringController.backUpCalled) {
            steeringController.lin_steering_algorithm(); // compute and publish twist commands and cmd_vel and cmd_vel_stamped
        }
        else if (steeringController.shouldBackUp) {
            steeringController.twist_cmd_.linear.x = -0.4;
            steeringController.twist_cmd_.linear.y = 0.0;
            steeringController.twist_cmd_.linear.z = 0.0;
            steeringController.twist_cmd_.angular.x = 0.0;
            steeringController.twist_cmd_.angular.y = 0.0;
            steeringController.twist_cmd_.angular.z = 0.0;
            steeringController.cmd_publisher_.publish(steeringController.twist_cmd_);
        }
        else {
            steeringController.twist_cmd_.linear.x = 0.0;
            steeringController.twist_cmd_.linear.y = 0.0;
            steeringController.twist_cmd_.linear.z = 0.0;
            steeringController.twist_cmd_.angular.x = 0.0;
            steeringController.twist_cmd_.angular.y = 0.0;
