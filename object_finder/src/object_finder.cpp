...
public:
    ObjectFinder(ros::NodeHandle* nodehandle, PclUtils* pclUtils); //define the body of the constructor outside of class definition
    ~ObjectFinder(void) {
    }
...
ObjectFinder::ObjectFinder(ros::NodeHandle* nodehandle, PclUtils* pclUtils): nh_(*nodehandle), pclUtils_(*pclUtils),
   object_finder_as_(nh_, "objectFinderActionServer", boost::bind(&ObjectFinder::executeCB, this, _1),false)/*,pclKinect_clr_ptr(new pcl::PointCloud<pcl::PointXYZRGB>),
   plane_pts_ptr(new pcl::PointCloud<pcl::PointXYZRGB>),selected_pts_cloud_ptr(new pcl::PointCloud<pcl::PointXYZ>),downsampled_kinect_ptr(new pcl::PointCloud<pcl::PointXYZRGB>),
   rough_table_cloud_ptr(new pcl::PointCloud<pcl::PointXYZRGB>),can_pts_cloud_ptr(new pcl::PointCloud<pcl::PointXYZRGB>),table_xyz_ptr(new pcl::PointCloud<pcl::PointXYZ>),
...
void ObjectFinder::executeCB(const actionlib::SimpleActionServer<object_finder::object_finderAction>::GoalConstPtr& goal) {
    cout << "------------------------------------------------" << endl;
    ...
    //*********************************************************************************************************************************//        
        ...
        if (can_exist) {
            ...
            find_result_.found_object_code = object_finder::object_finderResult::OBJECT_FOUND;
        }
        else {
        ...
        find_result_.found_object_code = object_finder::object_finderResult::OBJECT_NOT_RECOGNIZED;
        object_finder_as_.setAborted(find_result_);
    }
}
