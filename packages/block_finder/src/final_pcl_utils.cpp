...
FinalPclUtils::FinalPclUtils(ros::NodeHandle* nodehandle) :   nh_(*nodehandle),
...
void FinalPclUtils::fit_points_to_plane(pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud_ptr, Eigen::Vector3f &plane_normal, double &plane_dist) {
...
Eigen::Vector3f  FinalPclUtils::compute_centroid(pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud_ptr) {
...
//compute and return the centroid of a pointCloud
Eigen::Vector3f  FinalPclUtils::compute_centroid(pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_cloud_ptr) {
    Eigen::Vector3f centroid;
    Eigen::Vector3f cloud_pt;   
    int npts = input_cloud_ptr->points.size();    
    centroid<<0,0,0;
...
void FinalPclUtils::fit_xformed_selected_pts_to_plane(Eigen::Vector3f &plane_normal, double &plane_dist) {
...
Eigen::Affine3f FinalPclUtils::transformTFToEigen(const tf::Transform &t) {
    Eigen::Affine3f e;
...
void FinalPclUtils::transform_selected_points_cloud(Eigen::Affine3f A) {
...
void FinalPclUtils::get_transformed_selected_points(pcl::PointCloud<pcl::PointXYZ> & outputCloud ) {
...
void FinalPclUtils::get_gen_purpose_cloud(pcl::PointCloud<pcl::PointXYZ> & outputCloud ) {
...
void FinalPclUtils::example_pcl_operation() {
 ...
 */
void FinalPclUtils::findCoplanarPoints() {
    double tolerance = 0;
...
void FinalPclUtils::copy_indexed_pts_to_output_cloud(vector<int> &indices,PointCloud<pcl::PointXYZRGB> &outputCloud) {
...
Eigen::Vector3d FinalPclUtils::find_avg_color() {
...
void FinalPclUtils::find_indices_color_match(vector<int> &input_indices,
...
void FinalPclUtils::filter_cloud_z(double z_nom, double z_eps, 
...
void FinalPclUtils::find_coplanar_pts_z_height(double plane_height,double z_eps,vector<int> &indices) {
    filter_cloud_z(pclTransformed_ptr_,plane_height,z_eps,indices);
}

void FinalPclUtils::filter_cloud_z(PointCloud<pcl::PointXYZ>::Ptr inputCloud, double z_nom, double z_eps, vector<int> &indices) {
    int npts = inputCloud->points.size();
    Eigen::Vector3f pt;
    indices.clear();
    double dz;
...
void FinalPclUtils::filter_cloud_z(PointCloud<pcl::PointXYZ>::Ptr inputCloud, double z_nom, double z_eps, 
                double radius, Eigen::Vector3f centroid, vector<int> &indices)  {
    int npts = inputCloud->points.size();
    Eigen::Vector3f pt;
    indices.clear();
...
void FinalPclUtils::analyze_selected_points_color() {
    int npts = pclTransformedSelectedPoints_ptr_->points.size(); //number of points
    //copy_cloud(pclTransformedSelectedPoints_ptr_,pclGenPurposeCloud_ptr_); //now have a copy of the selected points in gen-purpose object
    //Eigen::Vector3f offset;
    //offset<<0,0,0.05;
...
void FinalPclUtils::copy_cloud(PointCloud<pcl::PointXYZ>::Ptr inputCloud, PointCloud<pcl::PointXYZ>::Ptr outputCloud) {
...
void FinalPclUtils::transform_cloud(Eigen::Affine3f A, pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud_ptr,
...
void FinalPclUtils::initializeSubscribers() {
    ROS_INFO("Initializing Subscribers");

    pointcloud_subscriber_ = nh_.subscribe("/kinect/depth/points", 1, &FinalPclUtils::kinectCB, this);
    // add more subscribers here, as needed

    // subscribe to "selected_points", which is published by Rviz tool
    selected_points_subscriber_ = nh_.subscribe<sensor_msgs::PointCloud2> ("/selected_points", 1, &FinalPclUtils::selectCB, this);
}

//member helper function to set up publishers;

void FinalPclUtils::initializePublishers() {
 ...
 */
void FinalPclUtils::kinectCB(const sensor_msgs::PointCloud2ConstPtr& cloud) {
...
void FinalPclUtils::selectCB(const sensor_msgs::PointCloud2ConstPtr& cloud) {
